...
class DoublyDecoderLayer(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        self.doubly_attn = MultiheadAttention(
            self.embed_dim,
            args.decoder_attention_heads,
            dropout=args.attention_dropout,
            encoder_decoder_attention=True,
    ...
    def forward(
        ...
        residual = x
        x = self.maybe_layer_norm(self.encoder_attn_layer_norm, x, before=True)
        if prev_doubly_attn_state is not None:
            if incremental_state is None:
                incremental_state = {}
            prev_key, prev_value = prev_doubly_attn_state[:2]
            saved_state = {"prev_key": prev_key, "prev_value": prev_value}
            if len(prev_doubly_attn_state) >= 3:
                saved_state["prev_key_padding_mask"] = prev_doubly_attn_state[2]
            self.doubly_attn._set_input_buffer(incremental_state, saved_state)
        x, attn = self.doubly_attn(
            query=x,
            key=visual_features,
...
class DoublyFusionDecoderLayer(nn.Module):
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        super().__init__()
        self.embed_dim = args.decoder_embed_dim
        self.cross_self_attention = getattr(args, 'cross_self_attention', False)
