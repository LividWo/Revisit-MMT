...
class LSTMEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        self.dense = nn.Linear(self.img_dim, hidden_size)
        self.sigmoid = nn.Sigmoid()
        self.gate_dense = nn.Linear(2 * hidden_size, hidden_size)
        self.args = args

        self.out = open(args.save_dir + '/analysis.out', 'w')

    def forward(self, src_tokens, src_lengths, visions):
        if self.left_pad:
        ...
        output = text_repr + gate * output  # for image, standard one
        # for g in gate:
...
@register_model_architecture('lstm_gated', 'lstm_gated')
