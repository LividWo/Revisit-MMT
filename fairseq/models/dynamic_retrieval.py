...
class CaptionImageRetriever(nn.Module):
    ...
    def forward(self, caption_input_ids, caption_segment_ids, caption_input_masks, labels=None):
        ...
        topk_scaled_log_prob = F.log_softmax(retrieved_dot_product, dim=-1)
        assert topk_scaled_log_prob.shape == topk_idx.shape
        return _, topk_scaled_log_prob, topk_idx


@register_model('dynamic')
class DynamicTransformerModel(FairseqEncoderDecoderModel):

    def __init__(self, args, encoder, decoder, retriever, bert_tokenizer):
    ...
    def build_model(cls, args, task):
        ...
        retriever = CaptionImageRetriever(bert_config, bert=bert_encoder, args=args)
        matcher_state_dict = torch.load(args.pretrained_retriever, map_location="cpu")

        retriever.load_state_dict(matcher_state_dict, strict=False)
        ...
        return cls(args, encoder, decoder, retriever, bert_tokenizer)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, bert_tokens, **kwargs):
        ...
        for i in range(self.args.topk):
            # print(topk_idx.shape, src_tokens.shape)
            # print(topk_idx[:, i].shape)
            encoder_out = self.encoder(
                src_tokens, src_lengths=src_lengths, topk_idx=topk_idx[:, i], **kwargs
            )
            ...
            retrieval_scores.append(topk_log_softmax[:, i])

        return decoder_outs, retrieval_scores
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(self, src_tokens, src_lengths, topk_idx, return_all_hiddens=False, **unused):
        ...
        batch_size = batch_image_ids.size()[0]
        # should not use 0, cause we have image0
        image_padding_mask = batch_image_ids.eq(-1)
        image_mask = ~image_padding_mask

        image_embedding = self.img_embeddings(batch_image_ids)  # B*img_dim
        image_embedding = image_embedding.view(batch_size, 1, self.img_dim)  # B, 1, img_dim

        text_repr = x.transpose(0, 1)  # T x B x C -> B x T x C
        image_repr = self.dense(image_embedding)  # B, 1, C

        b, t, c = text_repr.shape
        output = image_repr.expand(b, t, c)
...
@register_model_architecture('dynamic', 'dynamic')
...
@register_model_architecture('dynamic', 'dynamic_iwslt_de_en')
def dynamic_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('dynamic', 'dynamic_wmt_en_de')
def dynamic_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('dynamic', 'dynamic_tiny')
def dynamic_tiny(args):
...
@register_model_architecture('dynamic', 'dynamic_vatex')
def dynamic_vatex(args):
