...
class GatedMLPModel(FairseqEncoderDecoderModel):
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        print(args.save_dir + '/gated_mlp.txt')
        self.out = open(args.save_dir + '/norm_text.txt', 'w')
        self.norm_out = open(args.save_dir + '/norm_visual.txt', 'w')
    ...
    def forward(self, src_tokens, src_lengths, visions, cls_input=None, return_all_hiddens=False, **unused):
        ...
        for g in torch.norm(text_repr, dim=2)[mask]:
            print(g.flatten().tolist(), file=self.out)
        # output = (1 - gate) * text_repr + gate * output  # for video 
        output = text_repr + v_repr  # for image, standard one

        
        norm_div = torch.norm(v_repr, dim=2)
        ...
        for norm in norm_div:
            print(norm.flatten().tolist(), file=self.norm_out)
...
@register_model_architecture('gated_mlp', 'gated_mlp')
...
@register_model_architecture('gated_mlp', 'gated_mlp_iwslt_de_en')
...
@register_model_architecture('gated_mlp', 'gated_mlp_tiny')
...
@register_model_architecture('gated_mlp', 'gated_mlp_vatex')
